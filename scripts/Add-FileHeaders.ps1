<#
.SYNOPSIS
    Adds Aaron Stannard copyright headers to C# files that don't have them.

.DESCRIPTION
    Scans all .cs files in src/ and tests/ directories and adds the standard
    copyright header to files that are missing it.
...
function Get-FileHeader {
    ...
    return @"
// -----------------------------------------------------------------------
// <copyright file="$FileName" company="Aaron Stannard">
//      Copyright (C) $CreatedYear - $currentYear Aaron Stannard <https://github.com/Aaronontheweb>
// </copyright>
// -----------------------------------------------------------------------
...
function Test-HasHeader {
    ...
    return $Content -match "^\s*(//|/\*)\s*-{5,}" -or $Content -match "<copyright.*Aaron Stannard"
}
